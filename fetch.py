...
def fetch_versions(repo):
    ...
    try:
        ...
        try:
            ...
            sys.stderr.write(
                f"Could not get revision list for {repo['name']} {repo['owner']}\n"
            )
            return ret

    for rev in revs:
        try:
            e = ts.repositories.get_repository_revision_install_info(
                repo["name"], repo["owner"], rev
            )
        except bioblend.ConnectionError:
            time.sleep(10)
            try:
                ...
                sys.stderr.write(
                    f"Could not fetch the results of {repo['name']} {repo['owner']} {rev}\n"
                )
                continue
